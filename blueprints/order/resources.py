...
class OrderResource(Resource):
    ...
    def post(self):
        ...
        if args['qty'] < 0:
            return {'message': 'tidak boleh negatif'},303
    ...
    def patch(self, id):
        ...
        parser.add_argument("new_qty", type= int, help= 'action not exist', location= 'json', default=0)
        args = parser.parse_args()
        ...
        if args['action'] == "ubah_qty":
            ...
            selisih = args['new_qty'] - detail.qty
            detail.qty = args['new_qty']
            order.total_price += (price * selisih)
            order.updatedAt = db.func.current_timestamp()
            db.session.add(order)
            db.session.commit()
